...
if st.session_state.logged_in:
    ...
    if menu == "📊 Dashboard":
        ...
        if not runs_df.empty and 'Date' in runs_df.columns:
            ...
            # -------------------------------------------------------
            # CHART 1: SEASONAL VOLUME (stacked bar, fixed)
            # -------------------------------------------------------
            st.subheader("Seasonal Volume Trends")

            plot_df = (
                runs_df
                .groupby('Date_Only')[['Recovery_Min', 'LT1_Min', 'LT2_Min']]
                .sum()
                .reset_index()
            ...
            if not lac_df.empty and {'Heart_Rate', 'Pace', 'Date', 'Lactate_mmol'}.issubset(lac_df.columns):
                ...
                if len(test_dates) > 0:
                    ...
                    with col1:
                        baseline_date = st.selectbox(
                            "🔵 Baseline Test", options=test_dates,
                            index=suggested_idx,
                            format_func=lambda x: x.strftime('%b %d, %Y')
                        )
                    with col2:
                        comp_date = st.selectbox(
                            "🔴 Current Status", options=test_dates,
                            index=len(test_dates) - 1,
                            format_func=lambda x: x.strftime('%b %d, %Y')
                        )
                    ...
                    if show_ghost:
                        ...
                        with p_col1:
                            ...
                            planned_avg_mileage = curr_mileage + (
                                st.number_input("Avg. Weekly Mileage Change", -10, 30, 5) / 2
                            )
                        with p_col2:
                            lt2_min = st.number_input("Avg LT2 Minutes/Week", 0, 120, 50)
                            vol_qual = st.slider("Consistency (1-5)", 1.0, 5.0, 4.0, step=0.5)
                        ...
                        runs_df_ef = load_data(RUN_LOG)
                        ef_trend_coeff = 1.0
                        if len(runs_df_ef) >= 4:
                            last_4 = runs_df_ef.tail(4)
                        ...
                        shift_decimal = (
                            weeks_out * 1.2 * dim_coeff *
                            ((planned_avg_mileage / curr_mileage) ** 0.5) *
                            (vol_qual / 3.0) * ef_trend_coeff * pol_penalty
                        ) / 60.0

                    # --- DUAL Y-AXIS CHART: Pace + HR vs Lactate ---
                    # Each test date gets: pace on left y-axis, HR on right y-axis, lactate on x
                    fig_lac, ax_pace = plt.subplots(figsize=(10, 5))
                    ax_hr = ax_pace.twinx()

                    colors = {'baseline': 'blue', 'current': 'red'}

                    for t_date, label, color, ls in [
                        (baseline_date, 'Baseline', colors['baseline'], '--'),
                        (comp_date, 'Current', colors['current'], '-'),
                    ]:
                        day_df = lac_df[lac_df['Date'].dt.date == t_date].copy()
                        if day_df.empty:
                            continue
                        day_df = day_df.sort_values('Lactate_mmol')

                        # Pace curve (left y-axis) — invert so faster pace is higher
                        ax_pace.plot(
                            day_df['Lactate_mmol'], day_df['Pace_Dec'],
                            marker='o', linestyle=ls, color=color,
                            label=f'{label} Pace: {t_date}'
                        )

                        # HR curve (right y-axis)
                        ax_hr.plot(
                            day_df['Lactate_mmol'], day_df['Heart_Rate'],
                            marker='s', linestyle=ls, color=color, alpha=0.45,
                            label=f'{label} HR: {t_date}'
                        )

                    if show_ghost:
                        latest_df = lac_df[lac_df['Date'].dt.date == comp_date].sort_values('Lactate_mmol')
                        ax_pace.plot(
                            latest_df['Lactate_mmol'],
                            latest_df['Pace_Dec'] - shift_decimal,
                            marker='x', linestyle=':', color='purple', linewidth=2.5,
                            label='Projected Goal Pace'
                        )

                    # Axis labels & formatting
                    ax_pace.set_xlabel("Lactate (mmol/L)")
                    ax_pace.set_ylabel("Pace (Min/Mile)", color='black')
                    ax_hr.set_ylabel("Heart Rate (BPM)", color='gray')
                    ax_hr.tick_params(axis='y', labelcolor='gray')

                    # Invert pace axis so faster (lower decimal) appears higher on chart
                    ax_pace.invert_yaxis()
                    current_yticks = ax_pace.get_yticks()
                    ax_pace.set_yticklabels([format_pace(t) for t in current_yticks])

                    # Vertical threshold reference lines
                    ax_pace.axvline(x=2.0, color='green', linestyle=':', alpha=0.5, linewidth=1)
                    ax_pace.axvline(x=4.0, color='orange', linestyle=':', alpha=0.5, linewidth=1)
                    ax_pace.text(2.05, ax_pace.get_ylim()[0], 'LT1 ~2.0', fontsize=8, color='green', va='bottom')
                    ax_pace.text(4.05, ax_pace.get_ylim()[0], 'LT2 ~4.0', fontsize=8, color='orange', va='bottom')

                    ax_pace.grid(True, linestyle=':')

                    # Combined legend
                    lines_pace, labels_pace = ax_pace.get_legend_handles_labels()
                    lines_hr, labels_hr = ax_hr.get_legend_handles_labels()
                    ax_pace.legend(lines_pace + lines_hr, labels_pace + labels_hr, loc='upper left', fontsize=8)

                    plt.tight_layout()
                    st.pyplot(fig_lac)
            else:
                st.info("No lactate data logged yet. Use '🩸 Log Lactate Test' to add a step-test.")

            st.divider()

            # -------------------------------------------------------
            # CHART 3: FITNESS & FORM MODEL
            # -------------------------------------------------------
            ...
            if not fitness_df.empty:
                ...
                ax_fit.fill_between(fitness_df['Date'], 0, fitness_df['Form'],
                                    where=(fitness_df['Form'] >= 0), color='green', alpha=0.3, label='Fresh')
                ax_fit.fill_between(fitness_df['Date'], 0, fitness_df['Form'],
                                    where=(fitness_df['Form'] < 0), color='orange', alpha=0.3, label='Tired')
        ...
        with st.form("manual_run_form"):
            ...
            if submitted:
                new_run = pd.DataFrame([{
                    ...
                    'Recovery_Min': rec_min, 'LT1_Min': lt1_min, 'LT2_Min': lt2_min,
                    'VO2_Min': vo2_min, 'Aerobic_EF': 0.0,
                    'Athlete_ID': st.session_state.username
                }])
                existing_runs = load_data(RUN_LOG)
